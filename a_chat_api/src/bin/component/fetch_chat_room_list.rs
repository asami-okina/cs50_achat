...
// ②①の配列で数値の比較を行い、同じindexを使用して大元のvecの並び替えを行う
pub fn sort_last_message_created_at_list(all_result: &mut Vec<FetchChatRoomListResultEnumItem>, all_result_last_message_created_at_list:Vec<i32>) -> &mut Vec<FetchChatRoomListResultEnumItem> {
...
// SQL実行部分(Friends)
pub async fn fetch_chat_room_list_friend(pool: &MySqlPool, user_id: &str, search_text: Option<String>) -> anyhow::Result<FetchChatRoomListResultEnum> {
...
// SQL実行部分(Group)
pub async fn fetch_chat_room_list_group(pool: &MySqlPool, user_id: &str, search_text: Option<String>) -> anyhow::Result<FetchChatRoomListResultEnum> {
    let mut result_list = vec![];
    match search_text {
        Some(search_text) => {
        // search_textがある場合、user_idに紐づくチャットルーム一覧をsearch_textで更に絞る
...
pub async fn fetch_group_member_user_ids(pool: &MySqlPool,group_chat_room_id: &u64) -> anyhow::Result<Vec<String>> {
    let result = sqlx::query!(
        r#"
            SELECT
                user_id
...
// 該当userが関わるdirect_chat_room_idの取得
pub async fn fetch_chat_room_list_direct_chat_room_id(pool: &MySqlPool, user_id:&str) -> anyhow::Result<Vec<u64>>{
    let result = sqlx::query!(
        r#"
            SELECT
                direct_chat_room_id
...
// 該当userが関わるgroup_chat_room_idの取得
pub async fn fetch_chat_room_list_group_chat_room_id(pool: &MySqlPool, user_id:&str) -> anyhow::Result<Vec<u64>>{
    let result = sqlx::query!(
        r#"
            SELECT
                group_chat_room_id
...
pub async fn fetch_chat_room_list_own_last_read_time_friend(pool: &MySqlPool, user_id:&str, direct_chat_room_id: &u64) -> anyhow::Result<i32> {
    let result = sqlx::query!(
        r#"
            SELECT
                last_read_time
...
// 自分のuser_id、direct_chat_room_idでlast_read_timeを取得(Group)
pub async fn fetch_chat_room_list_own_last_read_time_group(pool: &MySqlPool, user_id:&str, group_chat_room_id: &u64) -> anyhow::Result<i32> {
    let result = sqlx::query!(
        r#"
            SELECT
                last_read_time
...
// 最終メッセージ情報の取得(Friend)
pub async fn fetch_chat_room_list_last_message_info_friend(pool: &MySqlPool, direct_chat_room_id: &u64) -> anyhow::Result<FetchChatRoomListLastMessageInfoFriendEnum>{
    let result = sqlx::query!(
        r#"
            SELECT
                direct_chat_room_id,
...
// 最終メッセージ情報の取得(Group)
pub async fn fetch_chat_room_list_last_message_info_group(pool: &MySqlPool, group_chat_room_id: &u64) -> anyhow::Result<FetchChatRoomListLastMessageInfoGroupEnum>{
    let result = sqlx::query!(
        r#"
            SELECT
                group_chat_room_id,
...
pub async fn fetch_chat_room_list_last_message_info_unread_count_friend(pool: &MySqlPool, direct_chat_room_id: &u64, &own_last_read_time: &i32, friend_user_id:&str) -> anyhow::Result<i64>{
    let result = sqlx::query!(
        r#"
            SELECT
...
pub async fn fetch_chat_room_list_last_message_info_unread_count_group(pool: &MySqlPool, group_chat_room_id: &u64, &own_last_read_time: &i32, own_user_id:&str) -> anyhow::Result<i64>{
    let result = sqlx::query!(
        r#"
            SELECT
                COUNT(*) as unread_count
...
// 友達の場合の戻り値を生成
pub async fn make_chat_room_list_result_list_friend(pool: &MySqlPool, direct_chat_room_id: &u64, &own_last_read_time:&i32, friend_user_id:&String, friend_nickname:Option<&String>,friend_profile_image: Option<&String> , mut result_list:Vec<FetchChatRoomListResultEnumItem>) -> anyhow::Result<Vec<FetchChatRoomListResultEnumItem>>{
     // 最終メッセージ情報の取得
