...
export function UserIdForm({
  ...
  // ユーザーIDアイコンのデフォルト表示
  const [defaultDisplayUserIcons, setDefaultDisplayUserIcons] =
    useState<boolean>(false);
  ...
  async function _isAvailableUserIdValidation() {
    try {
      ...
      const response = await fetch(
        API_SERVER_URL +
          `/api/signup/is_available_user_id_validation/${userIdText}`,
    ...
    <View>
      ...
      {pageType === "SignUp" ? (
        <UserIdFormDescription
          ...
          defaultDisplayUserIcons={defaultDisplayUserIcons}
        />
      ) : null}
    </View>
  );
